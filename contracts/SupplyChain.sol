...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier isOwner(address _address) {require (msg.sender == owner, 'This  address is no owner'); _;}
  modifier verifyCaller (address _address) {require (msg.sender == _address, 'Incorrect Caller'); _;}
  modifier paidEnough(uint _price) {require(msg.value >= _price, 'Not enough ETH send'); _;}
  ...
  modifier forSale(uint _sku) {
    require(items[_sku].state == State.ForSale && items[_sku].seller != address(0), 'Status does not match forSale');
    _;
  }
  modifier sold(uint _sku) {require(items[_sku].state == State.Sold, 'Status does not match sold'); _;}
  modifier shipped(uint _sku) {require(items[_sku].state == State.Shipped, 'Status does not match shipped'); _;}
  modifier received(uint _sku) {require(items[_sku].state == State.Received, 'Status does not match received'); _;}
